...
interface TaskGameSettings {
    gameId: string; // Can be a string like "DFR_R35"
    taskId: number;
    gameName: string;
    gridSize: number;
    shuffleMoves: number;
...
export default function TaskGameScreen() {
    ...
    const [gameSettings, setGameSettings] = useState<TaskGameSettings | null>(null);
    ...
    const initializeGame = useCallback(async () => {
        ...
        try {
            // Fetch game settings from the server for the next task
            const response = await axios.post(`${API_URL}/game/create`, {
                game_type: "task_completion",
                taskId: params.taskId // Assuming server still needs this to identify the task
            });
            const settings: TaskGameSettings = response.data;

            setGameSettings(settings);
            setPieces(generatePuzzle(settings.gridSize, settings.shuffleMoves));
            setMoves(0);
            startTimer();
        } catch (error) {
            console.error("Failed to start task game:", error);
            Alert.alert("Error", "Could not start the task game.", [{ text: "Go Back", onPress: () => router.back() }]);
        } finally {
            setLoading(false);
        }
    }, [params.taskId]);
    ...
    const handleGameWin = async (finalMoves: number, finalTime: number) => {
        ...
        try {
            ...
            Alert.alert("Error", "Could not save your result, but congratulations on winning!");
        }
    };
    ...
    const handlePiecePress = (pressedIndex: number) => {
        ...
        const { row: pressedRow, col: pressedCol } = { row: Math.floor(pressedIndex / gameSettings.gridSize), col: pressedIndex % gameSettings.gridSize };
        const { row: emptyRow, col: emptyCol } = { row: Math.floor(emptyIndex / gameSettings.gridSize), col: emptyIndex % gameSettings.gridSize };
    ...
    const gap = 12;
    const pieceSize = Math.floor((boardSize - (gameSettings.gridSize + 1) * gap) / gameSettings.gridSize);
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <Modal transparent={true} visible={winModalVisible} animationType="fade">
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContainer}>
                        ...
                        <Text style={styles.modalMessage}>You've completed the task!</Text>
                        ...
                        <View style={styles.modalActions}>
                            <Pressable style={[styles.modalButton, styles.modalButtonCancel]} onPress={() => router.back()}><Text style={styles.modalButtonTextCancel}>Back to Menu</Text></Pressable>
            ...
            <View style={styles.header}><Text style={styles.headerTitle}>Task Mode</Text></View>

            <View style={styles.mainContent}>
                <View style={styles.infoCard}>
                    <View style={styles.infoRow}><Text style={styles.infoLabel}>Task:</Text><Text style={styles.infoValue}>{gameSettings.gameName}</Text></View>
                    <View style={styles.infoRow}><Text style={styles.infoLabel}>Grid Size:</Text><Text style={styles.infoValue}>{`${gameSettings.gridSize} x ${gameSettings.gridSize}`}</Text></View>
                </View>
